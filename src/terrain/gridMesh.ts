...
export const createGridMesh = (width: number, height: number, spacing: number = 1, wireframe: boolean = false): GridMesh => {
    ...
    for (let z = 0; z < height; z++) {
        for (let x = 0; x < width; x++) {
            ...
            // Flat grid - heights will be calculated in GPU shader
            const heightY = 0.0;
    ...
    // Create placeholder normals - will be calculated in GPU shader
    const normals = new Float32Array(vertexCount * 3);
    for (let i = 0; i < vertexCount; i++) {
        normals[i * 3 + 0] = 0.0;  // X component
        normals[i * 3 + 1] = 1.0;  // Y component (pointing up)
